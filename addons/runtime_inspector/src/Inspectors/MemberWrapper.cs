...
public partial class MemberWrapper : MarginContainer
{
	...
	private PackedScene? GetInputScene(Type inputType)
	{
		...
		if (inputType.IsGenericType && inputType.GetGenericTypeDefinition() == typeof(System.Collections.Generic.Dictionary<,>))
			if (_inspectorScenes.Count > 9)
				return _inspectorScenes[9];
